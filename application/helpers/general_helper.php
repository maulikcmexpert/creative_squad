...
//         $CI = &get_instance();
//         $from = 'sahid.cmexpertise@gmail.com';
//         $to = $data['to'];
//         $subject = $data['subject'];

//         ob_start();
//         $data;
//         include(APPPATH. 'views/users/invoice/invoice_template.php');
...
//         $CI->email->clear();
//         $CI->email->from($config ['smtp_user'], "invoice");
//         $CI->email->to( $to );
//         if (isset($data ["bcc"])) {
...
function sendEmail($emailData)
{
    ...
    if (!empty($emailData)) {
        ...
        $config = array(
            ...
            'smtp_port' => 587,
            'smtp_user' =>  "vimal.cmexpertise@gmail.com",
            'smtp_pass' => "icwnsqbzuatszcmp",
        ...
        $from = isset($emailData['from']) ? $emailData['to'] : 'maulik.cmexpertise@gmail.com';
        $to = $emailData['to'];
        ...
        $ci->email->from($from, 'Cmexpertise');
        $ci->email->to($to);
        $ci->email->subject($subject);
        $ci->email->message($template);
        if ($ci->email->send()) {
